...
export default class IndustryStandardElementDetails extends PureComponent {
  ...
  render() {
    const {
      ...
      timeFormat, commentCollapsed, machineAllowance, standardElementId, comment, hasBetaAccess,
    } = this.props;

    const commentIconClasses = classNames(
      'clickable', 'fa',
      {
        'fa-comment-o': !comment && commentCollapsed,
        'fa-comment': !comment && !commentCollapsed,
        'fa-commenting-o': comment && commentCollapsed,
        'fa-commenting': comment && !commentCollapsed,
      });
...
IndustryStandardElementDetails.propTypes = {
