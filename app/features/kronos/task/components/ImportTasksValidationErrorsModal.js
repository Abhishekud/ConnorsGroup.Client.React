...
import {closeImportTasksValidationErrors} from '../actions';
import {
  showSelector,
...
class ImportTasksValidationErrorsModal extends Component {
  ...
  handleConfirm() {
    ...
    this.props.closeImportTasksValidationErrors();

    const processedMsg = `${totalRecordCount} record${(totalRecordCount === 1) ? ' was' : 's were'} processed.`;
    const createdMsg = `${createdRecordCount} ${(createdRecordCount === 1) ? 'Kronos Tasks has' : 'Kronos Tasks have'} been added.`;
    const updatedMsg = `${updatedRecordCount} ${(updatedRecordCount === 1) ? 'Kronos Tasks has' : 'Kronos Tasks have'} been updated.`;
...
export default connect(
  mapStateToProps,
  {
    closeImportTasksValidationErrors,
  }
