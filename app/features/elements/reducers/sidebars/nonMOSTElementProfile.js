...
import {
  ...
  EDIT_NON_MOST_ELEMENT,
  CANCEL_EDIT_NON_MOST_ELEMENT,
  SET_EDIT_NON_MOST_ELEMENT_WORKING_MODEL_PROPERTY,
  UPDATE_NON_MOST_ELEMENT_PENDING,
  UPDATE_NON_MOST_ELEMENT_FULFILLED,
  UPDATE_NON_MOST_ELEMENT_REJECTED,
  TOGGLE_NON_MOST_ELEMENT_PROFILE_SIDEBAR,
  TOGGLE_NON_MOST_ELEMENT_STATUS_LOG_COMMENT,
} from '../../actions';
import {
...
export default function (state = initialState, action) {
  switch (action.type) {
    ...
    case EDIT_NON_MOST_ELEMENT:
      return state.set('editing', true);

    case CANCEL_EDIT_NON_MOST_ELEMENT:
      return state.withMutations(map =>
        map.set('editing', false)
          .set('workingModel', state.get('pristineModel'))
          .set('validationErrors', Map()));

    case SET_EDIT_NON_MOST_ELEMENT_WORKING_MODEL_PROPERTY: {
      const {name, value} = action.payload;
      return state.setIn(['workingModel', name], value);
    }

    case UPDATE_NON_MOST_ELEMENT_PENDING:
      return state.set('saving', true);

    case UPDATE_NON_MOST_ELEMENT_FULFILLED: {
      const {
        id, name, elementUnitOfMeasureId, elementActivityId, status, elementStatusLogs, measuredTimeMeasurementUnits,
        elementUnitOfMeasureName, elementActivityName, applicatorId, applicatorName, lastEditorName,
    ...
    case UPDATE_NON_MOST_ELEMENT_REJECTED:
      return state.withMutations(map => {
        map.set('saving', false);
    ...
    case TOGGLE_NON_MOST_ELEMENT_PROFILE_SIDEBAR:
      return state.set('show', !state.get('show'));

    case TOGGLE_NON_MOST_ELEMENT_STATUS_LOG_COMMENT: {
